# PromptDock 1.2.0 (Build 7) — Early Access
...
- Fixed App Group authorization in distributed builds by using Xcode automatic signing, so existing local data opens normally.
...
- 改用 Xcode 自动签名，修复分发版本无法访问 App Group 资料库的问题，现有本地数据可正常打开。

## Upgrade safety / 升级安全

Build 7 preserves the existing `L96B6KHL5Y.PromptDock` App Group. It has been opened against the existing data store successfully, so no data migration is required. Exporting a JSON backup before replacing an Early Access build is still recommended.

Build 7 保留现有的 `L96B6KHL5Y.PromptDock` App Group，并已使用现有资料库完成实际启动验证，因此不需要迁移数据。替换 Early Access 版本前仍建议先导出一份 JSON 备份。
...
2. Quit PromptDock, open `PromptDock-1.2.0-build7.dmg`, and drag PromptDock into Applications to replace the previous build.
...
1. 需要 macOS 14 或更高版本。
2. 退出 PromptDock，打开 `PromptDock-1.2.0-build7.dmg`，将 PromptDock 拖入“应用程序”并替换旧版本。
...
The SHA-256 checksum is provided in `PromptDock-1.2.0-build7.dmg.sha256`.

SHA-256 校验值见 `PromptDock-1.2.0-build7.dmg.sha256`。
